...
namespace FollowBot
{
    public class DefenseAndFlaskTask : ITask
    {
        ...
        public async Task<bool> Run()
        {
            ...
            // 본인이 Grace Period 상태면(=입장 직후 아직 한 번도 움직이지 않았으면) 플라스크 사용 금지
        ...
        private static Player GetNextOnRotation(string linkWhitelist)
        {
            ...
            else
            {
                ...
                if (party.Count < 1)
                {
                    GlobalLog.Error("--------------------------------------------------------------------------------------------------------------------------------");
                    GlobalLog.Error(" ");
                    GlobalLog.Error($"[DefenseAndFlaskTask] CastOnParty is selected, but the LinkWhitelist have some problems (the name in the LinkWithelist do not match the party names [partyMembers] or not in the same zone), pls check the Defensive skill setting.!");
                    GlobalLog.Error(" ");
                    GlobalLog.Error("--------------------------------------------------------------------------------------------------------------------------------");
                }
